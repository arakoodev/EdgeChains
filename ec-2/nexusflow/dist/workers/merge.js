...
function inputStreamName(runId, parent, name) {
...
function mergeByPosition(a, b, deep) {
    ...
    for (let i = 0; i < len; i++) {
        ...
        if (item1 === undefined)
            ...
            out.push(deep ? deepMerge(item1, item2) : { ...item1, ...item2 });
    }
    return out;
}
function mergeByMatch(a, b, opts) {
    const usedB = new Set();
    const res = [];
...
class MergeWorker extends StreamAwareBullMQWorker {
    ...
    async processor(job) {
        ...
        if (job.name === "merge-root") {
            const { workflow_run_id, input1Name = "input1", input2Name = "input2", mode = "match", joinType = "keepMatches", field1 = "id", field2 = "id", fuzzyCompare = false, deepMerge = false, } = job.data;

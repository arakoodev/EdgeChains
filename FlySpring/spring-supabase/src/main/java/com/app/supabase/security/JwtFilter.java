...
public class JwtFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filter) throws ServletException, IOException {
        ...
        if (token != null && this.jwtHelper.validate(token)) {
            ...
            /** Here, we are getting user from Supabase API, so if user is disabled (even you can check for disabled as well by requesting DB)
             *  or token is revoked then instantly the user won't be
             * able to access internal APIs unlike with prior approach, the user will be able to access APIs until the token isn't expired..  */

...
public interface ChatCompletionLoggerService {
  ...
  Single<Page<ChatCompletionLog>> findAllByCallIdentifier(@Body HashMap<String, String> mapper, @Path("page") int page, @Path("size") int size);

  @POST(value = "logs/chat-completion/findByIdentifier/sorted/{page}/{size}")
  Single<Page<ChatCompletionLog>> findAllByCallIdentifierOrderByCompletedAtDesc(@Body HashMap<String, String> mapper, @Path("page") int page, @Path("size") int size);

  @POST(value = "logs/chat-completion/findByLatencyLessThanEq/{page}/{size}")
  Single<Page<ChatCompletionLog>> findAllByLatencyLessThanEqual(@Body HashMap<String, Long> mapper, @Path("page") int page, @Path("size") int size);

  @POST(value = "logs/chat-completion/findByLatencyGtrThanEq/{page}/{size}")
  Single<Page<ChatCompletionLog>> findAllByLatencyGreaterThanEqual(@Body HashMap<String, Long> mapper ,@Path("page") int page, @Path("size") int size);
}

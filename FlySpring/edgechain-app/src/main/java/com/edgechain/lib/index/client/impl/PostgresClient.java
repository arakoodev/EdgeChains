...
public class PostgresClient {
    ...
    return new EdgeChain<>(Observable.create(emitter -> {
      try {
        ...
        Integer embeddingId = this.repository.upsertEmbeddings(postgresEndpoint.getTableName(),
            input, postgresEndpoint.getFilename(), wordEmbeddings, this.namespace);
  ...
  public EdgeChain<List<PostgresWordEmbeddings>> query(WordEmbeddings wordEmbeddings,
      PostgresDistanceMetric metric, int topK, int probes) {
    ...
    return new EdgeChain<>(Observable.create(emitter -> {
      try {
        ...
        if (postgresEndpoint.getMetadataTableNames() == null) {
          List<Map<String, Object>> rows = this.repository.query(postgresEndpoint.getTableName(),
              this.namespace, probes, metric, wordEmbeddings, topK);
          ...
          for (int i = 0; i < numberOfMetadataTables; i++) {
            ...
            for (Map<String, Object> row : rows) {
              ...
              if (!metadataTableName.contains("_title_metadata")
                  && contextChunkIds.contains(metadataId))
                continue;

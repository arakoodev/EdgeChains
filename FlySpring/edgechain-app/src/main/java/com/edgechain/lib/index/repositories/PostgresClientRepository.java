  ...
  public List<Map<String, Object>> query(
    ...
    if(metric.equals(PostgresDistanceMetric.IP)){
      ...
      return jdbcTemplate.queryForList(
              String.format("SELECT id, raw_text, namespace, filename, timestamp, (embedding <#> '%s') * -1 AS score FROM %s WHERE namespace='%s' ORDER" +
      ...
      return jdbcTemplate.queryForList(
              String.format("SELECT id, raw_text, namespace, filename, timestamp, 1 - ( embedding <=> '%s') AS score FROM %s WHERE namespace='%s' ORDER" +
                              " BY score DESC LIMIT %s;",
                      embeddings,
                      tableName,
                      namespace,
      ...
      return jdbcTemplate.queryForList(
              String.format("SELECT id, raw_text, namespace, filename, timestamp, (embedding <-> '%s') AS score FROM %s WHERE namespace='%s' ORDER" +
                              " BY score ASC LIMIT %s;",
                      embeddings,
                      tableName,
                      namespace,
                      topK)

...
public class HistoryContextController {
  ...
  @PostMapping("/create")
  public Single<HistoryContext> create(@RequestParam("id") String id, @RequestBody Endpoint endpoint) {
  ...
  public Single<HistoryContext> put(@RequestBody ContextPutRequest request) {
      return this.contextClient.put(request.getId(), request.getResponse(),request.getEndpoint()).toSingle();
  }

package com.app.openai.prompt.impl;
...
public class CompletionPrompt implements PromptTemplate {
    ...
    public String getPrompt() {
        return "Answer the following questions as best you can. You have access to the following tools: requests_get, " +
                pluginResponse.getPlugin().getName_for_model() + "\n" +
                pluginResponse.getPlugin().getName_for_model() + ": " +
                String.format("Call this tool to get the OpenAPI spec (and usage guide) for interacting with the %s API ", pluginResponse.getPlugin().getName_for_human()) +
                String.format("You should only call this ONCE! What is the %s API useful for? ", pluginResponse.getPlugin().getName_for_human()) +

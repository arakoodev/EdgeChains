...
public class EmbeddingController {
    ...
    private static final String OPENAI_API_KEY = "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";
    ...
    public Observable<String> upsertPinecone(@RequestParam("file") MultipartFile file) {
        return new PDFEmbeddingService(
                ...
                new Endpoint(OPENAI_EMBEDDINGS_API, OPENAI_API_KEY, new ExponentialDelay(2,3,2, TimeUnit.SECONDS)),
                512
        ).upsert(file).getScheduledObservableWithoutRetry();
    }

    @PostMapping("/pinecone/delete")
    public Observable<String> deletePinecone() {
        return new PineconeEmbeddingService(new Endpoint(
                PINECONE_DELETE_API,
                PINECONE_API_KEY
        )).delete().getScheduledObservableWithoutRetry();
    }
    ...
    public Observable<String> queryPinecone(@RequestBody HashMap<String, String> mapper) {
        return new PineconeEmbeddingService(new Endpoint(PINECONE_QUERY_API, PINECONE_API_KEY))
                .predict(mapper.get("query"), 0.0, OPENAI_API_KEY).getScheduledObservableWithoutRetry();
    }

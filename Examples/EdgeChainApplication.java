...
public class EdgeChainApplication  {
  ...
  public static void main(String[] args)  {
    ...
    /** Lines to be removed (Creating RedisEnv.class) ***/
    System.setProperty(
  ...
  public class ExampleController {
    ...
    public void upsert(ArkRequest arkRequest) throws IOException {
      ...
      IntStream.range(0, arr.length).parallel().forEach(i -> retrieval.upsert(arr[i],namespace));
    }
    ...
    public ArkResponse query(ArkRequest arkRequest) {
      ...
      // This entire thing is a blocking/Non-streaming version;
      return new EdgeChain<>(embeddingEndpoint.getEmbeddings(query)) // Step 1: Generate embedding using OpenAI for provided input
              .transform(embeddings -> pineconeEndpoint.query(embeddings,namespace,topK))  // Step 2: Use the generated embeddings to perform similarity search
    ...
    public ArkResponse chatWithPinecone(ArkRequest arkRequest) {
      ...
      // Create RedisEndpoint (Will be passing RedisEnv to it);
      ...
      return new EdgeChain<>(embeddingEndpoint.getEmbeddings(query)) // Step 1: Generate embedding using OpenAI for provided input
              .transform(embeddings -> pineconeEndpoint.query(embeddings,namespace, topK)) // Step 2: Get topK queries from Pinecone
              ...
              .transform(queries -> {
                ...
                System.out.printf("Queries:%s", queries); // For Logging Purpose, later to be removed
...
//
                return chatEndpoint.getChatCompletion(loader.get("prompt")) // Pass the concatenated prompt to JsonnetLoader
                        /** Here is the interesting part; May be we will have a non-streaming or streaming response
                        ...
                        .doOnNext(chatCompletionResponse ->  {
                          ...
                          if(Objects.isNull(chatCompletionResponse.getChoices().get(0).getFinishReason())) {
                            ...
                            redisEndpoint.update(historyContext.getId(),
    ...
    public StringResponse delete(ArkRequest arkRequest) {
      String namespace = arkRequest.getQueryParam("namespace");

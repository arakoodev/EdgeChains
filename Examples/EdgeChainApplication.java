...
public class EdgeChainApplication {
  ...
  public class ExampleController {
    ...
    public ArkResponse queryPinecone(ArkRequest arkRequest) {
      ...
      JsonnetLoader loader =
              new FileJsonnetLoader("R:\\pinecone-query.jsonnet")
                      .put("keepMaxTokens", new JsonnetArgs(DataType.BOOLEAN, "true"))
                      .put("maxTokens", new JsonnetArgs(DataType.INTEGER, "4096"));
      ...
      return new EdgeChain<>(
              ...
              .transform(embeddings -> new EdgeChain<>(pineconeEndpoint.query(embeddings, topK)).get()) // Step 2: Block The Observable & Get the result from Pinecone(id,// scores)
              .transform(embeddingsQuery -> {
                ...
                while (iterator.hasNext()) {
                  ...
                  loader.put("keepContext", new JsonnetArgs(DataType.BOOLEAN, "true")).put("context", new JsonnetArgs(DataType.STRING, pinecone)) // Step 3: Concatenate the Prompt: ${Base Prompt} - // ${Pinecone Output}
                          .loadOrReload();
                  // Step 4: Now, pass the prompt to OpenAI ChatCompletion & Add it to the list which will be returned
                  resp.add(EdgeChain.fromObservable(chatEndpoint.getChatCompletion(loader.get("prompt"))).get()); // You can use both new EdgeChain<>() or EdgeChain.fromObservable()
                  // Wrap the Observable & get the data in a blocking way... Pass the concatenated prompt to ChatCompletion..
                }
                return resp;
              })
              .getArkResponse();
    ...
    public ArkResponse chatWithPinecone(ArkRequest arkRequest) {
      ...
      RedisHistoryContextEndpoint contextEndpoint = new RedisHistoryContextEndpoint(new FixedDelay(3,3, TimeUnit.SECONDS));
      HistoryContext historyContext = EdgeChain.fromObservable(contextEndpoint.get(contextId)).get();
      ...
      return new EdgeChain<>(
              ...
              .transform(
                      embeddings ->
                              EdgeChain.fromObservable(pineconeEndpoint.query(embeddings, topK)).get()) // Step 2: Block the Observables &  Get topK queries from Pinecone
              ...
              .transform(
                      mapper -> {
                        ...
                        return chatEndpoint.getChatCompletion(loader.get("prompt")) // Pass the concatenated prompt to JsonnetLoader
                                /**
                                 * Here is the interesting part; So, with ChatCompletion Stream we will have streaming
                                 * Therefore, we create a StringBuilder to append the response as we need to save in redis
                                 */
                                .doOnNext(
                                          ...
                                          if (chatCompletionResponse.getObject().equals("chat.completion.chunk")) {
                                            ...
                                            if (Objects.isNull(chatCompletionResponse.getChoices().get(0).getFinishReason())) {
                                              ...
                                              EdgeChain.fromObservable(
                                                      contextEndpoint.put( historyContext.getId(),
    ...
    public void upsertRedis(ArkRequest arkRequest) throws IOException {
       ...
       */
      RedisEndpoint redisEndpoint = new RedisEndpoint("vector_index", namespace, new ExponentialDelay(3,3, 2,TimeUnit.SECONDS));
    ...
    public ArkResponse queryRedis(ArkRequest arkRequest) {
      ...
      return new EdgeChain<>(
              embeddingEndpoint.getEmbeddings(
                      query)) // Step 1: Generate embedding using OpenAI for provided input
              .transform(
                      embeddings -> EdgeChain.fromObservable(redisEndpoint.query(embeddings, topK)).get()) // Step 2: Blocking Get the result from Redis(id, scores)
              .transform(
                      embeddingsQuery -> {
                        List<ChatCompletionResponse> resp = new ArrayList<>();
    ...
    public ArkResponse chatWithRedis(ArkRequest arkRequest) {
      ...
      RedisEndpoint redisEndpoint = new RedisEndpoint("vector_index", namespace, new FixedDelay(3,3,TimeUnit.SECONDS));
      OpenAiEndpoint embeddingEndpoint =
              new OpenAiEndpoint(
                      OPENAI_EMBEDDINGS_API,
                      OPENAI_AUTH_KEY,
      ...
      return new EdgeChain<>(
              ...
              .transform(
                      embeddings ->
                              EdgeChain.fromObservable(redisEndpoint.query(embeddings, topK)).get()) // Step 2: Block the Observables &  Get topK queries from Pinecone
              // Iterator over PineconeResponse & Get the ids;
              .transform(
                      embeddingsQuery -> { // Step 3: Now, we concatenate/join each query with "\n";
                        // let's say topK=5; then we concatenate them and pass to ChatCompletion
              ...
              .transform(
                      queries -> {
                        ...
                        return mapper;
                      }) // Step 4: Get the ChatHistory, and then we pass ChatHistory & RedisOutput to our JsonnetLoader
              .transform(
                      mapper -> {
                        loader
                                .put("keepHistory", new JsonnetArgs(DataType.BOOLEAN, "true"))

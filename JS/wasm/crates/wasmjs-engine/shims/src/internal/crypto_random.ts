...
import { ERR_INVALID_ARG_TYPE, ERR_OUT_OF_RANGE } from "./internal_errors";

import { Buffer, kMaxLength } from "./internal_buffer";

import { arrayBufferToUnsignedBigInt } from "./crypto_util";
...
export function generatePrime(
    ...
    const { safe, bigint, add, rem } = processGeneratePrimeOptions(options as GeneratePrimeOptions);
...
export function checkPrime(candidate: PrimeNum, callback: CheckPrimeCallback): void;
export function checkPrime(
    candidate: PrimeNum,

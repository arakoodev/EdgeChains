...
import { kFinalized, kHandle, kState, getArrayBufferOrView, getStringOption } from "./crypto_util";
...
import { validateEncoding, validateString, validateUint32 } from "./validators";

import { normalizeEncoding } from "./internal_utils";

import { isArrayBufferView, isCryptoKey, isAnyArrayBuffer } from "./internal_types";
...
let Hmac = function (
    ...
    } else if (typeof key !== "string" && !isArrayBufferView(key) && !isAnyArrayBuffer(key)) {
        throw new ERR_INVALID_ARG_TYPE(
            "key",
    ...
    new (hmac: string, key: ArrayLike | KeyObject | CryptoKey, options?: TransformOptions): Hmac;
};

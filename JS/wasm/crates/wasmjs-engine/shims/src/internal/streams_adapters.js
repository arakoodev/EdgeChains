/* eslint-disable */
import { Readable } from "./streams_readable";

import { TextEncoder, TextDecoder } from "@sinonjs/text-encoding";

import { Writable } from "./streams_writable";
...
import { createDeferredPromise, normalizeEncoding } from "./internal_utils";

import { validateBoolean, validateObject } from "./validators";
...
export function newStreamWritableFromWritableStream(writableStream, options = {}) {
    ...
    const writable = new Writable({
        ...
        writev(chunks, callback) {
            ...
            writer.ready.then(() => {
                return Promise.all(chunks.map((data) => writer.write(data))).then(done, done);
            }, done);
        },
...
export function newStreamDuplexFromReadableWritablePair(pair = {}, options = {}) {
    ...
    writer.closed.then(
        () => {
            writableClosed = true;
            if (!isWritableEnded(duplex)) destroy(duplex, new ERR_STREAM_PREMATURE_CLOSE());
        },
        (error) => {
            writableClosed = true;
            readableClosed = true;

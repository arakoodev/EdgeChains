  ...
  try {
    ...
    handle_file_copy({
      code: fs.readFileSync(
        path.join(
          root_dir_path,
          "__common/src/layouts/" + layout_file + ".ts",
        ),
        "utf8",
      ),
    ...
    handle_file_copy({
      code: fs.readFileSync(
        path.join(
          root_dir_path,
          "__common/src/routes/" + route_file + ".ts",
        ),
        "utf8",
      ),
    ...
    handle_file_copy({
      code: fs.readFileSync(
        path.join(
          root_dir_path,
          "__common/src/types/" + type_file + ".ts",
        ),
        "utf8",
      ),
    ...
    handle_file_copy({
      code: fs.readFileSync(
        path.join(
          root_dir_path,
          "__common/src/testGeneration/" + test_file + ".ts",
        ),
        "utf8",
      ),
      ...
      jsonnet_files.map(async (file) => {
        return handle_file_copy({
          code: fs.readFileSync(
            path.join(
              root_dir_path,
              "__common/src/jsonnet/" + file + ".jsonnet",
            ),
            "utf8",
          ),

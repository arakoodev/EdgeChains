...
const classText =
    "@RestResource(urlMapping='/classcontent')\n" +
...
async function getContent() {
    try {
        ...
        const initialResponse = await gpt3endpoint.gptFnTestGenerator(
            JSON.parse(prompt).promptStart
        );
        console.log("Initial Response.....\n\n" + initialResponse);
        var responce = await gpt3endpoint.gptFnTestGenerator(
            initialResponse + JSON.parse(testPrompt).prompt
        );
        ...
        responce = await gpt3endpoint.gptFnTestGenerator(
            JSON.parse(prompt).textConversion + "\n\n" + codeBlocks[0]
        );

...
export class HydeSearchService {
  ...
  async hydeSearchAdaEmbedding(arkRequest: ArkRequest) {
    try {
      ...
      const gptResponse = await this.gpt3endpoint.gptFn(prompt);
      ...
      const embeddingsListChain: Promise<number[][]> = Promise.all(
        gpt3Responses.map(async (resp) => {
          const embedding = await this.gpt3endpoint.embeddings(resp);
          return embedding;
        }),
      );
      ...
      const finalAnswer = await this.gpt3endpoint.gptFnChat(chatMessages);

...
export class Playwright {
  ...
  async #getThoughtAndCommands() {
    ...
    this.#setHistory({
      role: "assistant",
      content: openaiResponse.function_call.arguments,
    });
  ...
  async #interactWithPage(page: Page, task: string) {
    ...
    while (1) {
      ...
      if (command.name == "findInPage" || commandRunningAttempt >= 3) {
        ...
        this.#setHistory({
          role: "assistant",
          content: findInPageRespones.content,
        });
      ...
      const playwrightCode = JSON.parse(
        playwrightCodeResponse.function_call.arguments,
      ).code;
      if (this.verbose == true) {

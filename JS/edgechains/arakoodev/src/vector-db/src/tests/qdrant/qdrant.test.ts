...
describe("Qdrant", () => {
  ...
  it("should upsert points through the REST API", async () => {
    const client = {
      put: jest
        .fn()
        .mockResolvedValue({ data: { result: { status: "acknowledged" } } }),
    } as any;
  ...
  it("should search points through the REST API", async () => {
    const client = {
      post: jest
        .fn()
        .mockResolvedValue({ data: { result: [{ id: 1, score: 0.9 }] } }),
    } as any;
  ...
  it("should search points with a named vector through the REST API", async () => {
    const client = {
      post: jest
        .fn()
        .mockResolvedValue({ data: { result: [{ id: 1, score: 0.9 }] } }),
    ...
    const vector = { name: "content", vector: [0.1, 0.2, 0.3] };

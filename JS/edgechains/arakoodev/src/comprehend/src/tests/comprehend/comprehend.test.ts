...
vi.mock("@aws-sdk/client-comprehend", () => ({
    ComprehendClient: vi.fn().mockImplementation(() => ({ send: vi.fn() })),
...
describe("Comprehend", () => {
    ...
    describe("redact (chains detection + redaction)", () => {
        it("calls Comprehend and redacts the detected PII", async () => {
            ...
            (comprehend as any).client.send = vi.fn().mockResolvedValueOnce({
                Entities: [{ Type: "EMAIL", BeginOffset: 8, EndOffset: 22, Score: 0.99 }],
            });
